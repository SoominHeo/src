		...
		for engLine in list2:
			...
			for korWord in korLine:
				if korWord in engLine:
					count += 1
			tmp.append(count)
			#print(korLine+1,",",engLine+1," : ",set(list1[korLine]),"\n",set(list2[engLine]))
			#print(list(set(list1[korLine]) & set(list2[engLine])))
			#tmp.append(len(list(set(list1[korLine]) & set(list2[engLine]))))
		table.append(tmp)
	#for korIdx,korLine in enumerate(list1):
	#	for engIdx,engLine in enumerate(list2):
	#		print(korIdx,",",engIdx," : ",len(set(korLine) & set(engLine)))
	...
	for  x in range(length_kor):
		for y in range(length_eng):
			if x == 0 or y == 0:
				...
				#candidate.append(lengths[x][y]+1+table[x][y])
				#candidate.append(lengths[x+1][y] + table[x][y])
				#candidate.append(lengths[x][y+1] + table[x][y])
				lengths[x+1][y+1] += max(candidate) + table[x][y]
				#lengths[x+1][y+1] += max(candidate)
				#if x == 1 and y ==1:
					#print("test")
					#print(table[x][y])
					#print(max(candidate))
...
def check_answer(result,idx,subtype,distance_value,jaccard_value):
	answer_file = open("./../../data/Joongang/ANS/{idx}.csv".format(idx=idx),'r',encoding='utf8',errors='ignore')
	check_file = open("./../../data/Joongang/ANS/result/{subtype}/{distance_value}/{jaccard_value}/{index}.txt".format(subtype=subtype,index=idx, distance_value = distance_value, jaccard_value =jaccard_value),'w',encoding='utf8')
...
def check_directory(subtype, distance,value):
	ANS_path = "/../../data/Joongang/ANS/{subtype}/{distance}/{value}/"
	LCS_path = "/../../data/Joongang/LCS/{subtype}/{lang}/{distance}/{value}/"
	print("1")
